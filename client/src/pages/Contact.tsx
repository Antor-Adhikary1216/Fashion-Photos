...
const successDismissDelayMs = 5000
...
export function Contact() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const confirmationTarget = user
        ? user.email
        : 'the email address you entered'
      const successMessage = `Your booking request was received. A confirmation email was sent to ${confirmationTarget}.`
  ...
  return (
    <>
      <AuthSuccessPopup message={successPopupMessage} />
      <main>
        <PageHeader
          eyebrow="Booking"
        ...
        <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-20 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          >
            ...
            {message ? (
              <p className="text-sm text-gold-200">{message}</p>
            ) : null}
